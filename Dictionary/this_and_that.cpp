...
// 특정 값 제거 패턴
정렬 후
vec.erase(remove(vec.begin(), vec.end(), "특정 값"), vec.end()); 
...
// ⚡ 에라토스테네스의 체란? <<< 소수를 찾을 때 자주 쓰는 방법
"소수가 아닌 수를 지워가면서 남은 수가 소수다!" 라는 개념이야.

👇 순서
처음에 2부터 n까지 모두 소수라고 생각하고 true 표시

2부터 시작해서, 2의 배수 전부 제거 -> 다음 소수 3 → 3의 배수 전부 제거 -> 다음 소수 5 → 5의 배수 제거 ... 이런 식으로
...
🧠 언제 쓰나?
	...
	최댓값/최솟값을 구하는 문제에서 값의 범위를 기준으로 탐색할 때
	→ 예: 랜선 자르기 문제 (너가 푼 문제!), 예산 배분, 기타 최적화 문제
